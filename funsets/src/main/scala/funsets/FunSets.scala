...
object FunSets {
   ...
   */
    ...
    def iter(a: Int): Boolean = {
      if (a > bound) true
      else if (a <= bound && contains(s, a) && !contains(p, a)) false
      else iter(a+1)
    }
    iter(-999)
  }
   ...
   * that satisfies `p`.
   */
    def exists(s: Set, p: Int => Boolean): Boolean = !forall(s, (a: Int) => !contains(p, a))
  
  /**
   * Returns a set transformed by applying `f` to each element of `s`.
   */
    def map(s: Set, f: Int => Int): Set = (a: Int) => exists(s, (x: Int) => f(x) == a)
